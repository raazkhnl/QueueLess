...
const formatHistory = (action: string) => ({
  ...
  linked_to_appointment: 'Linked to appointment',
...
export default function TicketTracking() {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAuthenticated) return toast.error('Please log in to reply on this ticket');
  ...
  const handleReopen = async () => {
    if (!reopenReason.trim()) return toast.error('Reason is required to reopen');
    try {
  ...
  if (error || !ticket) {
    return (
      <div className="max-w-3xl mx-auto py-8 px-4 text-center">
        ...
        <h2 className="text-2xl font-bold font-display">{error || 'Something went wrong'}</h2>
        <Link to="/" className="text-indigo-600 hover:underline mt-4 inline-block">Return to homepage</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <div className="flex items-center justify-between mb-4">
        <Link to="/" className="btn-ghost"><ArrowLeft className="w-4 h-4 mr-2" /> Back</Link>
      ...
      <div className="card p-6 border-t-4 border-indigo-600">
        <div className="flex justify-between items-start mb-6 flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">{ticket.organization?.name}{ticket.branch?.name ? ` · ${ticket.branch.name}` : ''}</p>
            {ticket.externalSubmissionNo && (
              <p className="text-[11px] mt-1">
                External ref: <span className="font-mono font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded">{ticket.externalSubmissionNo}</span>
                {ticket.sourceSystem ? <span className="text-slate-500"> via {ticket.sourceSystem}</span> : null}
              </p>
            )}
          </div>
          <div className="text-right">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_COLORS[ticket.status] || 'bg-slate-100 text-slate-700'}`}>
              {formatStatus(ticket.status)}
            </span>
            {ticket.priority && (
              <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-wider">Priority: <strong className="text-slate-700">{ticket.priority}</strong></p>
            )}
            {ticket.slaDueDate && !isTerminal && (
              <p className="text-[11px] text-slate-500 mt-1">SLA: {new Date(ticket.slaDueDate).toLocaleString()}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-slate-50 p-4 rounded-lg mb-6">
          <h3 className="font-semibold text-slate-700 mb-1">{ticket.issueType?.name || 'General'}</h3>
          <p className="text-slate-600 text-sm whitespace-pre-wrap">{ticket.description}</p>
          {ticket.resolutionNote && isTerminal && (
            <div className="mt-3 pt-3 border-t border-slate-200">
              <p className="text-xs uppercase tracking-wider text-emerald-700 font-semibold mb-1">Resolution</p>
        ...
        {showReopen && (
          <div className="mb-6 p-4 border border-rose-200 bg-rose-50/50 rounded-lg">
            <label className="block text-sm font-medium mb-1">Why are you reopening?</label>
            <textarea
              rows={2} className="input-field" value={reopenReason}
              onChange={(e) => setReopenReason(e.target.value)}
              placeholder="The issue is not actually resolved because…"
            />
            <div className="flex gap-2 mt-2 justify-end">
              <button onClick={() => setShowReopen(false)} className="btn-ghost btn-sm">Cancel</button>
              <button onClick={handleReopen} className="btn-primary btn-sm">Reopen</button>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4 mb-8">
          <h3 className="font-semibold text-lg flex items-center gap-2">
            <Send className="w-4 h-4 text-indigo-600" /> Conversation
          </h3>
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-sm font-medium text-slate-800">
                      ...
                      {c.author?.role && c.author.role !== 'citizen' && (
                        <span className="ml-2 text-[10px] uppercase bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded">{c.author.role.replace('_',' ')}</span>
                      )}
                      {c.isInternal && (
                        <span className="ml-2 text-[10px] uppercase bg-amber-200 text-amber-800 px-1.5 py-0.5 rounded">Internal</span>
                      )}
                    </p>
          ...
          {isAuthenticated ? (
            ...
            <p className="text-xs text-slate-500">
              <Link to={`/login?redirect=/issue/track/${ticket.refCode}`} className="text-indigo-600 hover:underline">Log in</Link> to reply on this ticket.
            </p>
          )}
        </div>
        ...
        <div className="space-y-3">
          <h3 className="font-semibold text-lg flex items-center gap-2">
            <Clock className="w-4 h-4 text-indigo-600" /> Activity
          </h3>
          <div className="border-l-2 border-slate-200 ml-3 pl-4 space-y-3">
            {(ticket.history || []).map((h: any, idx: number) => (
              <div key={idx} className="relative">
                ...
                <p className="font-medium text-slate-800 text-sm">{formatHistory(h.action)}{h.toStatus ? ` → ${formatStatus(h.toStatus)}` : ''}</p>
                {h.reason && <p className="text-xs text-slate-600">{h.reason}</p>}
                {(h.actor?.name || h.actorName) && <p className="text-[11px] text-slate-400">by {h.actor?.name || h.actorName}</p>}
              </div>
            ))}
          </div>
        </div>
        ...
        {ticket.linkedAppointments?.length > 0 && (
          <div className="mt-8 border-t border-slate-100 pt-6">
            <h4 className="font-medium text-slate-700 flex items-center gap-2 mb-3">
              <Info className="w-4 h-4 text-slate-400" /> Linked appointments
            </h4>
            <div className="flex flex-wrap gap-2">
              {ticket.linkedAppointments.map((apt: any) => (
                <Link key={apt._id} to={`/appointments/${apt.refCode}`}
                  className="bg-white border border-slate-200 p-3 rounded shadow-sm hover:border-indigo-300 transition-colors">
                  <p className="text-[10px] text-slate-500 uppercase">Appointment</p>
                  <p className="font-mono font-bold text-slate-800 text-sm">{apt.refCode}</p>
                  {apt.status && <p className="text-[10px] text-slate-500 capitalize">{formatStatus(apt.status)}</p>}
                </Link>
              ))}
            </div>
          </div>
